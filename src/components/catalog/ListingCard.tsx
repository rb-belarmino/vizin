...
async function handleShare(listing: Listing) {
  const url = window.location.origin + `/?q=${encodeURIComponent(listing.title)}`;
  const shareData = {
    title: listing.title,
    ...
    <Card className="overflow-hidden flex flex-col h-full group hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300">
      ...
      <div className="relative w-full h-48 bg-muted overflow-hidden">
        {listing.portfolioImageUrl && !imgError ? (
          ...
          <div className="flex flex-col items-center justify-center w-full h-full brand-gradient gap-2">
            <span className="text-4xl" role="img" aria-label={listing.categoryId}>{emoji}</span>
            <span className="text-white/70 text-xs font-medium">{listing.categoryId}</span>
          </div>
        )}
      ...
      <CardHeader className="pb-2 pt-4">
        <div className="flex items-start justify-between gap-2">
          <CardTitle className="text-base leading-tight line-clamp-2 flex-1">{listing.title}</CardTitle>
          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold border shrink-0 ${badgeColor}`}>
          ...
          <div className="flex items-center gap-2 w-full">
            ...
            <span className="text-xs text-muted-foreground font-medium truncate">
              ...
              {listing.providerApartmentId && (
                <span className="text-muted-foreground/60 ml-1">· Apto {listing.providerApartmentId}</span>
              )}
            </span>
          </div>
        )}
